...
const POINT_DISTANCE_X_MAX:float = 300.0

const POINT_DISTANCE_Y_MIN:float = -100.0
const POINT_DISTANCE_Y_MAX:float = 100.0

# Generate n points, each 50-200 away from each other
# Each point has two states, only 50-150 +-y
	...
	for i in range(num_points):
		...
		if i == 0:
			...
			var next_position: = Vector2(
				...
				previous_position.y + randf_range(POINT_DISTANCE_Y_MIN, POINT_DISTANCE_Y_MAX))
...
func generate_animations(container:Node2D) -> void:
	...
	for point in points:
		...
		var curve_position: = Vector2(
			point.position.x,
			point.position.y + randf_range(POINT_DISTANCE_Y_MIN, POINT_DISTANCE_Y_MAX))
